...
if ( ! function_exists( 'lumex_setup' ) ) :
	...
	function lumex_setup() {
		...
		register_nav_menus( array(
			'menu-1' => esc_html__( 'Primary', 'lumex' ),
		) );
		...
		add_theme_support( 'custom-background', apply_filters( 'lumex_custom_background_args', array(
...
function lumex_content_width() {
	...
	$GLOBALS['content_width'] = apply_filters( 'lumex_content_width', 640 );
}
...
function lumex_widgets_footer() {
    register_sidebar( array(
        'name'          => esc_html__( 'Footer', 'lumex' ),
        'id'            => 'footer',
        'description'   => esc_html__( 'Add widgets here.', 'lumex' ),
        'before_widget' => '<section id="%1$s" class="widget %2$s">',
        'after_widget'  => '</section>',
...
add_action( 'widgets_init', 'lumex_widgets_footer' );

/**
 * Enqueue scripts and styles.
 */
function lumex_scripts() {
	wp_enqueue_style( 'lumex-style', get_stylesheet_uri() );

	wp_enqueue_script( 'lumex-navigation', get_template_directory_uri() . '/js/navigation.js', array(), '20151215', true );

	wp_enqueue_script( 'lumex-skip-link-focus-fix', get_template_directory_uri() . '/js/skip-link-focus-fix.js', array(), '20151215', true );
